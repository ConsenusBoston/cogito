window.leverJobsOptions = { accountName: 'cogitocorp', includeCss: false };
window.loadLeverJobs = function (options) {
  ...
  function createJobs(responseData) {
    ...
    for (var i = 0; i < groupedPostings.length; i++) {
      ...
      if (haveDepartments) {
        content += '<section id="' + groupedPostings[i].departmentTitle.toLowerCase().replace(/\s+/g, '-') + '" class="lever-department" data-department="' + groupedPostings[i].departmentTitle + '"><h3 class="lever-department-title">' + sanitizeForHTML(groupedPostings[i].departmentTitle) + '</h3>';
      };

      for (j = 0; j < groupedPostings[i].teams.length; j++) {

        // Uncomment in order to have subheader
        ...
        content += '<ul class="lever-team" data-team="' + groupedPostings[i].teams[j].teamTitle + '">';
